...
const BookCard = ({ book }) => {
  ...
  return <article className="book-card">
    <Link to={`/books/${book.id}`} className="book-cover-wrap">
      {!imageFailed && book.imageUrl ? <img src={resolveImageUrl(book.imageUrl)} alt={`Bia sach ${book.title}`} className="book-card-img" onError={() => setImageFailed(true)} /> : <FallbackCover book={book} />}
